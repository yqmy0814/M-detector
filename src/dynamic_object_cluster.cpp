...
void DynObjCluster::Clusterprocess(std::vector<int> &dyn_tag,
  ...
  pcl::PointCloud<PointType>::Ptr cloud_clean_ptr(
      new pcl::PointCloud<PointType>);
  cloud_clean_ptr = event_point.makeShared();
  bbox_t bbox_high;
  ClusterAndTrack(dyn_tag, cloud_clean_ptr, pub_pcl_before_high, header,
                  pub_pcl_after_high, cluster_vis_high, predict_path_high,
                  bbox_high, delta_t, raw_point);
...
void DynObjCluster::ClusterAndTrack(
    std::vector<int> &dyn_tag, pcl::PointCloud<PointType>::Ptr &points_in,
    ros::Publisher points_in_msg, std_msgs::Header header_in,\ 
                    ros::Publisher points_out_msg,
    ros::Publisher cluster_vis, ros::Publisher predict_path, bbox_t &bbox,
    double delta, const pcl::PointCloud<PointType> &raw_point) {
  ...
  PubClusterResult_voxel(dyn_tag, header_in, bbox, delta, voxel_clusters,
                         raw_point, used_map_set);
}

void DynObjCluster::GetClusterResult(
...
void DynObjCluster::GetClusterResult_voxel(
    pcl::PointCloud<PointType>::Ptr points_in,
...
void DynObjCluster::PubClusterResult_voxel(
    std::vector<int> &dyn_tag, std_msgs::Header current_header, bbox_t &bbox,
    double delta, std::vector<std::vector<int>> &voxel_clusters,
    const pcl::PointCloud<PointType> &raw_point,
    std::unordered_set<int> &used_map_set) {
  int j = 0;
  ...
  ros::Time t0 = ros::Time::now();
  for (auto it = voxel_clusters.begin(); it != voxel_clusters.end();
       it++, j++) {
    Eigen::Vector3f xyz;
  ...
  for (int ite = 0; ite < raw_point.size(); ite++) {
    ...
    if (umap_ground[voxel].bbox_index > -1) {
      bbox.Ground_points[umap_ground[voxel].bbox_index].push_back(
          raw_point[ite]);
  ...
  double cluster_time =
      (ros::Time::now() - cluster_begin).toSec() - total_region_growth_time;
  if (out_file != "") out << cluster_time << std::endl;
}

bool DynObjCluster::ground_estimate(
    const pcl::PointCloud<PointType> &ground_pcl,
    const Eigen::Vector3f &world_z, Eigen::Vector3f &ground_norm,
    Eigen::Vector4f &ground_plane, pcl::PointCloud<PointType> &true_ground,
      ...
      if (esti_plane(plane, split_pcl) && plane[3] < thershold) {
        ...
        if (normvec.cross(world_z).norm() < sin(max_angle_from_body)) {
          ...
          for (int j = 0; j < ground_pcl.size(); j++) {
            ...
            float dis =
                fabs(point.dot(plane.head(3)) + 1.0f) / plane.head(3).norm();
            if (dis < thershold) {
  ...
  if (ground_pcl.size() > 0 &&
    ...
    for (int j = 0; j < ground_pcl.size(); j++) {
      ...
      point[1] = ground_pcl[j].y;
      point[2] = ground_pcl[j].z;
      float dis = fabs(point.dot(ground_plane.head(3)) + 1.0f) /
...
void DynObjCluster::ground_remove(const Eigen::Vector4f &ground_plane,
...
void DynObjCluster::isolate_remove(pcl::PointCloud<PointType> &cluster_pcl,
                                   std::vector<int> &cluster_pcl_ind,
                                   std::vector<int> &dyn_tag) {
  ...
  for (int i = 0; i < max_voxel_num; i++) {
    ...
    for (int j = 0; j < umap_cluster[voxel]->cloud->size(); j++) {
      ...
      new_cluster_pcl_ind.push_back(
          cluster_pcl_ind[umap_cluster[voxel]->cloud_index->at(j)]);
      dyn_index.insert(
          cluster_pcl_ind[umap_cluster[voxel]->cloud_index->at(j)]);
    }
  }
...
void DynObjCluster::oobb_estimate(const VoxelMap &vmap,
  ...
  if (max_award > 0) {
    for (int ite = 0; ite < NormVectorMap[max_ind].size(); ite++) {
      direction_main = direction_main + NormVectorMap[max_ind][ite].head(3) *
                                            PointSizeList[max_ind][ite] /
                                            NormVectorMap[max_ind][ite](3);
    }
  ...
  if (sec_award > 0) {
    for (int ite = 0; ite < NormVectorMap[sec_ind].size(); ite++) {
      direction_aux = direction_aux + NormVectorMap[sec_ind][ite].head(3) *
                                          PointSizeList[sec_ind][ite] /
                                          NormVectorMap[sec_ind][ite](3);
    }
  ...
  if (ground_norm.norm() < 0.1) {
    ...
    R.col(1) =
        (direction_aux - direction_aux.dot(R.col(0)) * R.col(0)).normalized();
    ...
    R.col(1) =
        (direction_main - direction_main.dot(R.col(0)) * R.col(0)).normalized();
    R.col(2) = (R.col(0).cross(R.col(1))).normalized();
  }
  ...
  for (int pit = 0; pit < points.size(); pit++) {
    ...
    if (project[0] < x_min) x_min = project[0];
    if (project[1] < y_min) y_min = project[1];
    if (project[2] < z_min) z_min = project[2];
    if (project[0] > x_max) x_max = project[0];
    if (project[1] > y_max) y_max = project[1];
    if (project[2] > z_max) z_max = project[2];
  }
...
void DynObjCluster::event_extend(const Eigen::Matrix3f &R, bool ground_detect,
                                 bbox_t &bbox, std::vector<int> &dyn_tag,
  ...
  for (int i = 0; i < bbox.Ground_voxels_vec[bbox_index].size(); i++) {
    ...
    if (bbox.Ground_voxels_set[bbox_index].count(voxel_cur) &&
      ...
      for (int ind = 0; ind < 6; ind++) {
        int voxel_neighbor =
            voxel_cur + x_ind[ind] * GridMapedgesize_xy * GridMapedgesize_z +
            y_ind[ind] * GridMapedgesize_z + z_ind[ind];
        if (voxel_neighbor < 0 || voxel_neighbor > GridMapsize) continue;
        if ((umap_insidebox[voxel_neighbor].bbox_index > -1 &&
             umap_insidebox[voxel_neighbor].points_num > 0 &&
             umap_insidebox[voxel_neighbor].bbox_index == bbox_index) ||
            (umap[voxel_neighbor].points_num > 0 &&
             umap[voxel_neighbor].cloud->size() > 0 &&
             umap[voxel_neighbor].bbox_index == bbox_index)) {
          pcl::PointCloud<PointType> points = *(umap_ground[voxel_cur].cloud);
          Eigen::Vector4f plane;
          if (ground_detect) {
            if (esti_plane(plane, points) &&
                abs(R.col(0).dot(plane.head(3).normalized())) < 0.8f) {
              umap[voxel_cur].bbox_index = bbox_index;
              umap[voxel_cur].cloud = umap_ground[voxel_cur].cloud;
              bbox.Point_cloud[bbox_index] += points;
              for (int j = 0; j < umap_ground[voxel_cur].cloud_index->size();
                   j++) {
                dyn_tag[umap_ground[voxel_cur].cloud_index->at(j)] = 1;
                bbox.Point_indices[bbox_index].push_back(
                    umap_ground[voxel_cur].cloud_index->at(j));
              }
              break;
            }
          }
...
bool DynObjCluster::esti_plane(Eigen::Vector4f &pca_result,
