...
class VOXEL {
 public:
  ...
  VOXEL(int64_t vx = 0, int64_t vy = 0, int64_t vz = 0) : x(vx), y(vy), z(vz) {}
...
struct Point_Cloud {
  ...
  Point_Cloud(int index) { this->bbox_index = index; }
...
struct test_struct {
  ...
  test_struct(int index) { bbox_index = index; }
...
class VOXEL_CLUSTER {
 public:
  ...
  void setVoxelResolution(float voxel_length, float edge_size_xy,
                          float edge_size_z,
  ...
  void setExtendRange(int range) { max_range = range; }

  void setMinClusterSize(int min_cluster_voxels) {
    min_cluster_voxels_ = min_cluster_voxels;
  }

  void createVoxelMap(std::vector<Point_Cloud> &umap_in,
    ...
    for (int i = 0; i < points_.size(); i++) {
      ...
      tmp.intensity = 0;
      // VOXEL voxel(floor(tmp.x/Voxel_revolusion),
      // floor(tmp.y/Voxel_revolusion), floor(tmp.z/Voxel_revolusion));
      int position =
          floor((tmp.x - xyz_origin(0)) / Voxel_revolusion) *
              Grid_edge_size_xy * Grid_edge_size_z +
          floor((tmp.y - xyz_origin(1)) / Voxel_revolusion) * Grid_edge_size_z +
          floor((tmp.z - xyz_origin(2)) / Voxel_revolusion);
      if (position < 0 ||
  ...
  void extendVoxelNeighbor(int voxel, std::unordered_set<int> &voxel_added) {
    for (int x_neighbor = -max_range; x_neighbor <= max_range; x_neighbor++) {
      for (int y_neighbor = -max_range; y_neighbor <= max_range; y_neighbor++) {
        for (int z_neighbor = -max_range; z_neighbor <= max_range;
             z_neighbor++) {
          float neighbor_dis =
              sqrt(x_neighbor * x_neighbor + y_neighbor * y_neighbor +
                   z_neighbor * z_neighbor);
          if (neighbor_dis - (float)max_range < 0.001f) {
            int voxel_neighbor =
                voxel + x_neighbor * Grid_edge_size_xy * Grid_edge_size_z +
                y_neighbor * Grid_edge_size_z + z_neighbor;
            if (voxel_neighbor < 0 || voxel_neighbor > Grid_edge_size_xy *
                                                           Grid_edge_size_xy *
                                                           Grid_edge_size_z)
              continue;
            if (voxel_set.count(voxel_neighbor) &&
                !voxel_added.count(voxel_neighbor)) {
              // std::cout << "voxel neightbor: " << voxel_neighbor <<
              // std::endl;
              voxel_added.insert(voxel_neighbor);
              // std::cout << "after: " << std::endl;
              // for(auto ite = voxel_added.begin(); ite!=voxel_added.end();
              // ite++)
              // {
              //     std::cout << *ite << " ";
              // }
    ...
    // {
    //     VOXEL voxel_neighbor(voxel.x + x_neighbor[neighbor_ind], voxel.y +
    //     y_neighbor[neighbor_ind], voxel.z + z_neighbor[neighbor_ind]); if
    //     (umap.count(voxel_neighbor) && !voxel_added.count(voxel_neighbor))
    //     {
    //         voxel_added.insert(voxel_neighbor);
    //         extendVoxelNeighbor(voxel_neighbor, voxel_added);
    //     }
  ...
  void extract(std::vector<std::vector<int>> &voxel_clusters) {
    ...
    for (int voxel_ind = 0; voxel_ind < voxel_list.size(); voxel_ind++) {
      ...
      if (voxel_set.count(voxel_cur)) {
        ...
        for (auto iter = voxel_added.begin(); iter != voxel_added.end();
             ++iter) {
